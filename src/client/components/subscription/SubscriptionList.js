  ...
  _fetchSubscriptions = async () => {
    ...
    return new Promise((resolve) => {
      if (server.id) {
        ...
        resolve(fetch(`/api/servers/${server.id}/subscriptions/`)
          .then(res => res.json())
          ...
          <Card.Body>
            ...
            <Row className="mb-3">
              <Col xs="auto" className="align-self-center">
                <Button title="Add Subscription" onClick={this.handleAddSubscription} disabled={addIsDisabled} server={server}>
                  {'+'}
                </Button>
              </Col>
              <Col>
            ...
            {subscriptions.map(val => (
              <SubscriptionItem key={val.id} subscription={val} server={server} />
            ))}

          </Card.Body>
        </Card>
        <SubsciptionEdit action="Add" modalShow={modalShow} handleClose={handleClose} server={server} />
      </React.Fragment>
    );
  }
}
