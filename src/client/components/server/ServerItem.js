...
import {
  Card, DropdownButton, Dropdown, Row, Col
...
export default class ServerItem extends React.Component {
  ...
  handleDelete = async (key, event) => {
    ...
    console.log(event);
    const postResponse = await axios.delete(`./api/servers/${server.id}`);
  ...
  render() {
    ...
    return (
      <React.Fragment>
          ...
          <Card.Body className="pr-1">
            <Row className="mr0">
              <Col xs="auto" className="p-0">
                <DropdownButton onClick={e => e.stopPropagation()} type="" title="" size="sm" variant="outline-primary" className="p-0 m-0">
        ...
        <ServerEdit action="Edit" modalShow={modalShow} handleClose={handleClose} form={server} />
      </React.Fragment>
    );
  }
}
